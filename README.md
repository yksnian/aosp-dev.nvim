...
| java.exclude_globs | {^prebuilts/sdk/sdk_} | Lua-pattern exclusions matched against the path relative to .intermediates/. Anchoring with ^ targets a top-level directory precisely, e.g. { "^external/cronet/", "^vendor/xxx/packages/apps/" } to drop the cronet network libs and vendor-specific apps |
